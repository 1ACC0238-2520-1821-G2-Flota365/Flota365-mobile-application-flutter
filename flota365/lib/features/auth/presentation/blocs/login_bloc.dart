...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  Future<void> _onSubmitted(LoginSubmitted event, Emitter<LoginState> emit) async {
    if (state.email.isEmpty || state.password.length < 6) {
      emit(state.copyWith(status: Status.failure, error: 'Datos inválidos.'));
      return;
    }

    emit(state.copyWith(status: Status.loading, error: null));
    try {
      final (user, _) = await _repo.login(state.email, state.password);
      ...
      emit(state.copyWith(status: Status.failure, error: msg.toString()));
    } catch (e) {
